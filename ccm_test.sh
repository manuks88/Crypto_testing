...
function ccm_test()
{
	...
	for (( iter=1;iter<$user_input;iter++ ))
	{
		...
		do
		{
			...
			ecmd="$KCAPI -x $ciphertype $aligned $stream -e -c \"$AEAD_name\" -n \"$nonce\" -k \"$key\" -a \"$assoc\" -p \"$data_item\" -l \"$taglen\""
			
			enc=$($KCAPI -x $ciphertype $aligned $stream -e -c "$AEAD_name" -n "$nonce" -k "$key" -a "$assoc" -p "$data_item" -l "$taglen" 2> /dev/null)
